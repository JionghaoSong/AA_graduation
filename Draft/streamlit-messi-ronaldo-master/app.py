...
def get_data(foot):
    ...
    messi_events_data_df['label'] = messi_events_data_df['label'].apply(
        lambda x: bytes(x, encoding='utf-8').decode('unicode-escape'))
    ronaldo_events_data_df['label'] = ronaldo_events_data_df['label'].apply(
        lambda x: bytes(x, encoding='utf-8').decode('unicode-escape'))
    ...
    barca_matches_dates_df['match'] = barca_matches_dates_df['match'].apply(
        lambda x: bytes(x, encoding='utf-8').decode('unicode-escape'))
    real_matches_dates_df['match'] = real_matches_dates_df['match'].apply(
        lambda x: bytes(x, encoding='utf-8').decode('unicode-escape'))
...
def plot_shots(messi_events_data_df, ronaldo_events_data_df, barca_matches_dates_df, real_matches_dates_df):
    ...
    messi_stats_df = pd.DataFrame(data=zip(messi_stats[:, 'Shot'].index, messi_stats[:, 'Shot']),
                                  columns=['match', '#shots'])
    ronaldo_stats = ronaldo_events_data_df.groupby(['label', 'eventName']).count()['eventId']
    ronaldo_stats_df = pd.DataFrame(data=zip(ronaldo_stats[:, 'Shot'].index, ronaldo_stats[:, 'Shot']),
                                    columns=['match', '#shots'])
    # Adding Dates
    messi_stats_df = pd.merge(messi_stats_df, barca_matches_dates_df, on='match', copy=False, how="left")
    ronaldo_stats_df = pd.merge(ronaldo_stats_df, real_matches_dates_df, on='match', copy=False, how="left")
...
def plot_free_kicks(messi_events_data_df, ronaldo_events_data_df, barca_matches_dates_df, real_matches_dates_df):
    ...
    try:
        messi_stats = messi_events_data_df.groupby(['label', 'eventName']).count()['eventId']
        messi_stats_df = pd.DataFrame(data=zip(messi_stats[:, 'Free Kick'].index, messi_stats[:, 'Free Kick']),
                                      columns=['match', '#free kicks'])
    except:
        messi_stats_df = pd.DataFrame(columns=['match', '#free kicks'])
    try:
        ronaldo_stats = ronaldo_events_data_df.groupby(['label', 'eventName']).count()['eventId']
        ronaldo_stats_df = pd.DataFrame(data=zip(ronaldo_stats[:, 'Free Kick'].index, ronaldo_stats[:, 'Free Kick']),
                                        columns=['match', '#free kicks'])
    except:
...
def plot_passes(messi_events_data_df, ronaldo_events_data_df, barca_matches_dates_df, real_matches_dates_df):
    ...
    # Table
    messi_stats = messi_events_data_df.groupby(['label', 'eventName']).count()['eventId']
    messi_stats_df = pd.DataFrame(data=zip(messi_stats[:, 'Pass'].index, messi_stats[:, 'Pass']),
                                  columns=['match', '#passes'])
    ronaldo_stats = ronaldo_events_data_df.groupby(['label', 'eventName']).count()['eventId']
    ronaldo_stats_df = pd.DataFrame(data=zip(ronaldo_stats[:, 'Pass'].index, ronaldo_stats[:, 'Pass']),
                                    columns=['match', '#passes'])
    # Adding Dates
    messi_stats_df = pd.merge(messi_stats_df, barca_matches_dates_df, on='match', copy=False, how="left")
    ronaldo_stats_df = pd.merge(ronaldo_stats_df, real_matches_dates_df, on='match', copy=False, how="left")
...
if __name__ == '__main__':
    ...
    tabs = bokeh.models.Tabs(
        tabs=[
            ...
            plot_free_kicks(messi_events_data_df, ronaldo_events_data_df, barca_matches_dates_df,
