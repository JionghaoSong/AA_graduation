...
def soccer_player_classification(test_size):
    ...
    plt.plot(test_data_class_name, test_data_count, marker='o', markerfacecolor='blue', markersize=6, color='skyblue',
             linewidth=2, label='Test')
    plt.plot(test_data_class_name, predicted_data_count, marker='o', markerfacecolor='orange', markersize=6,
...
def classify_player(PlayerName):
    ...
    for input_column in input_columns_dict:
        if input_columns_players.__contains__(input_column) != True:
            dummied_playerinputdata.insert(loc=len(dummied_playerinputdata.columns), column=input_column,
                                           value=input_columns_dict[input_column])
